...
**Only** add if it's specifically about the Desktop application or `epoch web` view. **Do not** add for terminal, TUI, or general epoch issues.
...
**Only** add if the issue mentions "zen" or "epoch zen" or "epoch black".

If the issue doesn't have "zen" or "epoch black" in it then don't add zen label

#### core

Use for core server issues in `packages/epoch/`, excluding `packages/epoch/src/cli/cmd/tui/`.
...
TUI (`packages/epoch/src/cli/cmd/tui/...`):
...
Core (`packages/epoch/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for epoch server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
