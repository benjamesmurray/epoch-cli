...
const reg = process.env.REGISTRY ?? "ghcr.io/benjamesmurray"
...
const setup = async () => {
  ...
  if (list.includes("epochcli")) {
    await $`docker buildx use epochcli`
    return
  }
  await $`docker buildx create --name epochcli --use`
}
