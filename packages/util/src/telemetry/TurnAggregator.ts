import type { LogLine, Turn, ToolExecution } from "./types";
import { isStartGenerate, isEndGenerate, isContextLimitReached, extractMetrics, parseLogLine, getSemanticToolName } from "./ParserUtils";
...
export class TurnAggregator {
  ...
  processLine(line: string | LogLine): Turn | null {
    ...
    if (isContextLimitReached(logLine)) {
      if (this.currentTurn) {
    ...
    if (isStartGenerate(logLine)) {
      if (logLine.metadata?.contextLimit) {
        this.lastContextLimit = parseInt(logLine.metadata.contextLimit, 10) || DEFAULT_CONTEXT_LIMIT;
      }
      return this.startNewTurn(logLine.timestamp, logLine.metadata?.phase, logLine.metadata?.activeAgent, logLine);
    }

    if (this.currentTurn) {
      this.currentTurn.lines.push(logLine);
      
      if (isEndGenerate(logLine)) {
        const metrics = extractMetrics(logLine);
        return this.endCurrentTurn(logLine.timestamp, metrics);
      }
    }
    ...
    if (this.currentTurn) {
        ...
        if (event.event === "TOOL_START") {
            const tool: ToolExecution = {
                callID: event.callID || `call_${Math.random().toString(36).slice(2, 11)}`,
                name: getSemanticToolName(event.tool, event.input),
                input: event.input,
                status: "running",
                startTime: timestamp,
            };
            this.currentTurn.tools.push(tool);
        }

        if (event.event === "TOOL_END") {
          const toolName = getSemanticToolName(event.tool, event.input);
          const tool = this.currentTurn.tools.find(t => 
              t.callID === event.callID || 
              (t.name === toolName && t.status === "running")
          );
          if (tool) {
              tool.status = event.status === "completed" ? "completed" : "failed";
              tool.output = event.output;
              tool.error = event.error;
              tool.endTime = timestamp;
          } else {
              // If we missed TOOL_START, create a finished one
