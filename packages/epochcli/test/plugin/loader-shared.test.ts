...
const disableDefault = process.env.EPOCHCLI_DISABLE_DEFAULT_PLUGINS
process.env.EPOCHCLI_DISABLE_DEFAULT_PLUGINS = "1"
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.EPOCHCLI_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.EPOCHCLI_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
describe("plugin.loader.shared", () => {
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["epochcli-openai-codex-auth@1.0.0", "epochcli-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("epochcli-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("epochcli-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("publishes session.error when install fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: ["broken-plugin@9.9.9"] }, null, 2))
      },
    })
  ...
  test("publishes session.error when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: [file] }, null, 2))
  ...
  test("publishes session.error when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: [missing] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "epochcli.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.EPOCHCLI_PURE
    process.env.EPOCHCLI_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.EPOCHCLI_PURE
      } else {
        process.env.EPOCHCLI_PURE = pure
      }
    }
  })
