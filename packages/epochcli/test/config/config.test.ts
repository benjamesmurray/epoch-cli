...
const managedConfigDir = process.env.EPOCHCLI_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "epochcli.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "epochcli.json") {
...
test("ignores legacy tui keys in epochcli config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://epochcli.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "epochcli.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["EPOCHCLI_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { epochcli: { options: { apiKey: "{env:EPOCHCLI_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["epochcli"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["EPOCHCLI_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["EPOCHCLI_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "epochcli.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .epochcli directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const epochcliDir = path.join(dir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })
      const agentDir = path.join(epochcliDir, "agent")
...
test("loads agents from .epochcli/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const epochcliDir = path.join(dir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })

      const agentsDir = path.join(epochcliDir, "agents")
...
test("loads commands from .epochcli/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const epochcliDir = path.join(dir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })

      const commandDir = path.join(epochcliDir, "command")
...
test("loads commands from .epochcli/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const epochcliDir = path.join(dir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })

      const commandsDir = path.join(epochcliDir, "commands")
...
test("does not try to install dependencies in read-only EPOCHCLI_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.EPOCHCLI_CONFIG_DIR
  process.env.EPOCHCLI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.EPOCHCLI_CONFIG_DIR
    else process.env.EPOCHCLI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable EPOCHCLI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
  ...
  const install = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    const mod = path.join(dir, "node_modules", "@epoch-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@epoch-ai/plugin", version: "1.0.0" }),
    )
  })
...
test("dedupes concurrent config dependency installs for the same dir", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (d: string) => {
    ...
    const mod = path.join(d, "node_modules", "@epoch-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@epoch-ai/plugin", version: "1.0.0" }),
...
test("serializes config dependency installs across dirs", async () => {
  ...
  const run = spyOn(Npm, "install").mockImplementation(async (dir: string) => {
    ...
    const mod = path.join(cwd, "node_modules", "@epoch-ai", "plugin")
    await fs.mkdir(mod, { recursive: true })
    await Filesystem.write(
      path.join(mod, "package.json"),
      JSON.stringify({ name: "@epoch-ai/plugin", version: "1.0.0" }),
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "epochcli.json"),
        JSON.stringify({ $schema: "https://epochcli.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .epochcli config
      const projectDir = path.join(dir, "project")
      const epochcliDir = path.join(projectDir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })
      ...
      // Local .epochcli config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .epochcli config with some overlapping plugins
      await Filesystem.write(
        path.join(epochcliDir, "epochcli.json"),
        JSON.stringify({
          $schema: "https://epochcli.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".epochcli")
...
// Note: preload.ts sets EPOCHCLI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .epochcli config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .epochcli directory config enables it
      const epochcliDir = path.join(dir, ".epochcli")
      await fs.mkdir(epochcliDir, { recursive: true })
      await Filesystem.write(
        path.join(epochcliDir, "epochcli.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/epochcli")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/epochcli")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "epochcli.json")
    expect(await Config.resolvePluginSpec("oh-my-epochcli@2.4.3", file)).toBe("oh-my-epochcli@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-epochcli@2.4.3", "file:///project/.epochcli/plugin/oh-my-epochcli.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.epochcli/plugin/demo.ts", "file:///project/.epochcli/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.epochcli/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const epochcliDir = path.join(projectDir, ".epochcli")
        const pluginDir = path.join(epochcliDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("EPOCHCLI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"]
    process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .epochcli/ directories when flag is set", async () => {
    const originalEnv = process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"]
    process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .epochcli directory with a command
          const epochcliDir = path.join(dir, ".epochcli", "command")
          await fs.mkdir(epochcliDir, { recursive: true })
          await Filesystem.write(path.join(epochcliDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .epochcli should NOT be in directories list
          const hasProjectEpoch = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectEpoch).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["EPOCHCLI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["EPOCHCLI_CONFIG_DIR"]
      process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["EPOCHCLI_CONFIG_DIR"]
      } else {
        process.env["EPOCHCLI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("EPOCHCLI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["EPOCHCLI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["EPOCHCLI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["EPOCHCLI_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from EPOCHCLI_CONFIG_DIR, not project
...
describe("EPOCHCLI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in EPOCHCLI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["EPOCHCLI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["EPOCHCLI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://epochcli.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["EPOCHCLI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["EPOCHCLI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in EPOCHCLI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["EPOCHCLI_CONFIG_CONTENT"]
...
test("parseManagedPlist strips MDM metadata keys", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({
      PayloadDisplayName: "Epoch CLI Managed",
      PayloadIdentifier: "ai.epochcli.managed.test",
      PayloadType: "ai.epochcli.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = await Config.parseManagedPlist(
    JSON.stringify({ $schema: "https://epochcli.ai/config.json" }),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://epochcli.ai/config.json")
})
