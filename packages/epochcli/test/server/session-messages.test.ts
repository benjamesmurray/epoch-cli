...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.EPOCHCLI_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.EPOCHCLI_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.EPOCHCLI_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.EPOCHCLI_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}
