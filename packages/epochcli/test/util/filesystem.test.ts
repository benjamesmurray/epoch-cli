...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "epochcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "epochcli.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "epochcli.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "epochcli.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["epochcli.json", "epochcli.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "epochcli.json"),
        path.join(tmp.path, "epochcli.jsonc"),
        path.join(project, "epochcli.json"),
        path.join(project, "epochcli.jsonc"),
      ])
    })
  })
