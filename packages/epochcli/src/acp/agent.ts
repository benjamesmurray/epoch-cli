...
import type { AssistantMessage, Event, EpochClient, SessionMessageResponse, ToolPart } from "@epoch-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: EpochClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `epochcli auth login` in the terminal",
        name: "Login with epochcli",
        id: "epochcli-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Epoch CLI stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const epochcliProvider = providers.find((p) => p.id === "epochcli")
    if (epochcliProvider) {
      if (epochcliProvider.models["big-pickle"]) {
        return { providerID: ProviderID.epochcli, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(epochcliProvider.models))
      if (best) {
        return {
