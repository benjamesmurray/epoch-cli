...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for epochcli.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal epochcli sessions
...
epochcli acp --cwd /path/to/project
```
...
```bash
EPOCHCLI_ENABLE_QUESTION_TOOL=1 epochcli acp
```
...
- **Terminal Integration**: Full terminal support via epochcli's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | epochcli acp
```
...
### Mapping to Epoch CLI

ACP sessions map cleanly to epochcli's internal session model:
