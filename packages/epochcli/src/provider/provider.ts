...
export namespace Provider {
  ...
  function e2eURL() {
    const url = Env.get("EPOCHCLI_E2E_LLM_URL")
  ...
  function custom(dep: CustomDep): Record<string, CustomLoader> {
    return {
      ...
      epochcli: Effect.fnUntraced(function* (input: Info) {
        const env = Env.all()
        const hasKey = iife(() => {
        ...
        const ok =
          ...
          Boolean((yield* dep.config()).provider?.["epochcli"]?.options?.apiKey)
      ...
      "amazon-bedrock": Effect.fnUntraced(function* () {
        ...
        return {
          ...
          async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
            ...
            // 1. options.region from epochcli.json provider config
      ...
      openrouter: () =>
        Effect.succeed({
          ...
          options: {
            headers: {
              "HTTP-Referer": "https://epochcli.ai/",
              "X-Title": "epochcli",
            },
          },
        }),
      ...
      gitlab: Effect.fnUntraced(function* (input: Info) {
        ...
        const aiGatewayHeaders = {
          "User-Agent": `epochcli/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      "cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
        ...
        return {
          ...
          options: {
            apiKey,
            headers: {
              "User-Agent": `epochcli/${Installation.VERSION} cloudflare-workers-ai (${os.platform()} ${os.release()}; ${os.arch()})`,
            },
          },
          async getModel(sdk: any, modelID: string) {
            return sdk.languageModel(modelID)
      ...
      "cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
        ...
        if (!apiToken) {
          throw new Error(
            "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
              "Set it via environment variable or run `epochcli auth cloudflare-ai-gateway`.",
          )
        }
        ...
        const opts = {
          ...
          headers: {
            "User-Agent": `epochcli/${Installation.VERSION} cloudflare-ai-gateway (${os.platform()} ${os.release()}; ${os.arch()})`,
          },
        }
      ...
      cerebras: () =>
        Effect.succeed({
          ...
          options: {
            headers: {
              "X-Cerebras-3rd-Party-Integration": "epochcli",
            },
          },
        }),
      kilo: () =>
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@epochcli/Provider") {}
  ...
  const layer: Layer.Layer<Service, never, Config.Service | Auth.Service | Plugin.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          for (const [id, provider] of Object.entries(providers)) {
            ...
            for (const [modelID, model] of Object.entries(provider.models)) {
              ...
              if (model.status === "alpha" && !Flag.EPOCHCLI_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
      ...
      const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
        ...
        if (providerID.startsWith("epochcli")) {
          priority = ["gpt-5-nano"]
        }
