...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run epochcli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for epochcli session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found epochcli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["epochcli", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting epochcli...")
        UI.println()

        const epochcliArgs = sessionId ? ["-s", sessionId] : []
        const epochcliProcess = Process.spawn(["epochcli", ...epochcliArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await epochcliProcess.exited
        if (code !== 0) throw new Error(`epochcli exited with code ${code}`)
      },
    })
  },
})
