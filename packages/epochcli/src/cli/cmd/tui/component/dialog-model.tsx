...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "epochcli" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
        ...
        return [
          {
            ...
            category,
            disabled: provider.id === "epochcli" && model.id.includes("-nano"),
            footer: model.cost?.input === 0 && provider.id === "epochcli" ? "Free" : undefined,
            onSelect: () => {
        ...
        pipe(
          ...
          map(([model, info]) => ({
            ...
            category: connected()
              ...
              : undefined,
            disabled: provider.id === "epochcli" && model.includes("-nano"),
            footer: info.cost?.input === 0 && provider.id === "epochcli" ? "Free" : undefined,
            onSelect() {
