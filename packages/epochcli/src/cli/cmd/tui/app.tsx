...
function rendererConfig(_config: TuiConfig.Info): CliRendererConfig {
  const mouseEnabled = !Flag.EPOCHCLI_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.EPOCHCLI_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Epoch CLI")
      return
    }
  ...
  command.register(() => [
    ...
    ...(Flag.EPOCHCLI_EXPERIMENTAL_WORKSPACES
      ? [
          {
    ...
    },
    {
      ...
      value: "docs.open",
      onSelect: () => {
        open("https://epochcli.ai/docs").catch(() => {})
        dialog.clear()
      },
      category: "System",
    },
  ...
  sdk.event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Epoch CLI v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.EPOCHCLI_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.EPOCHCLI_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
