...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".epochcli"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".epochcli", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): Config.PluginOrigin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".epochcli", "tui.json"),
  }
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const records = Flag.EPOCHCLI_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.EPOCHCLI_PURE && config.plugin_origins?.length) {
