...
import epochcli from "./theme/epochcli.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@epoch-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "epochcli")
        draft.active = typeof active === "string" ? active : "epochcli"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.epochcli, store.mode)
    })
