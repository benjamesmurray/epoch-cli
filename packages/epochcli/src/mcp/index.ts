...
export namespace MCP {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@epochcli/MCP") {}
      ...
      const connectTransport = (transport: Transport, timeout: number) =>
        Effect.acquireUseRelease(
          ...
          (t) =>
            Effect.tryPromise({
              try: () => {
                const client = new Client({ name: "epochcli", version: Installation.VERSION })
              ...
              if (isAuthError) {
                ...
                if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                  ...
                  return bus
                    .publish(TuiEvent.ToastShow, {
                      title: "MCP Authentication Required",
                      message: `Server "${key}" requires authentication. Run: epochcli mcp auth ${key}`,
                      variant: "warning",
                      duration: 8000,
                    })
                    .pipe(Effect.ignore, Effect.as(undefined))
