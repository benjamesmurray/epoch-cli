...
export namespace Instruction {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@epochcli/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.EPOCHCLI_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no EPOCHCLI_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.EPOCHCLI_CONFIG_DIR, Flag.EPOCHCLI_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
