  ...
  export function ControlPlaneRoutes(upgrade: UpgradeWebSocket, app = new Hono(), opts?: { cors?: string[] }): Hono {
    return app
      ...
      .use((c, next) => {
        ...
        const password = Flag.EPOCHCLI_SERVER_PASSWORD
        if (!password) return next()
        const username = Flag.EPOCHCLI_SERVER_USERNAME ?? "epochcli"
      ...
      .use(
        cors({
          ...
          origin(input) {
            ...
            if (/^https:\/\/([a-z0-9-]+\.)*epochcli\.ai$/.test(input)) return input
