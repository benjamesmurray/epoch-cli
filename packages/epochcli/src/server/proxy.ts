...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-epochcli-directory")
  out.delete("x-epochcli-workspace")
  if (!extra) return out
...
const app = lazy(() =>
  new Hono().get(
    "/__workspace_ws",
    upgradeWebSocket((c) => {
      const url = c.req.header("x-epochcli-proxy-url")
      const queue: Msg[] = []
...
export namespace ServerProxy {
  ...
  export function websocket(target: Extract<Target, { type: "remote" }>, req: Request, env: unknown) {
    ...
    next.set("x-epochcli-proxy-url", socket(target.url))
    return app().fetch(
      new Request(url, {
        method: req.method,
        headers: next,
