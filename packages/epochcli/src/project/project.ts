...
export namespace Project {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@epochcli/Project") {}
  ...
  > = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const fakeVcs = Info.shape.vcs.parse(Flag.EPOCHCLI_FAKE_VCS)
      ...
      const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
        return yield* fs.readFileString(pathSvc.join(dir, "epochcli")).pipe(
          Effect.map((x) => x.trim()),
      ...
      const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
        ...
        const data: DiscoveryResult = yield* Effect.gen(function* () {
          ...
          if (!id) {
            ...
            if (id) {
              yield* fs.writeFileString(pathSvc.join(worktree, ".git", "epochcli"), id).pipe(Effect.ignore)
            }
          }
        ...
        if (Flag.EPOCHCLI_EXPERIMENTAL_ICON_DISCOVERY)
