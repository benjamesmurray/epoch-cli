...
export const DEPRECATED_PLUGIN_PACKAGES = ["epochcli-openai-codex-auth", "epochcli-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, epochcliVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(epochcliVersion) || semver.major(epochcliVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.epochcli
  if (typeof range !== "string") return
  if (!semver.satisfies(epochcliVersion, range)) {
    throw new Error(`Plugin requires epochcli ${range} but running ${epochcliVersion}`)
  }
}
