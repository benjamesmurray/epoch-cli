import type { Hooks, PluginInput, Plugin as PluginInstance, PluginModule } from "@epoch-ai/plugin"
...
export namespace Plugin {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@epochcli/Plugin") {}
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const client = createEpochClient({
            ...
            headers: Flag.EPOCHCLI_SERVER_PASSWORD
              ? {
                  Authorization: `Basic ${Buffer.from(`${Flag.EPOCHCLI_SERVER_USERNAME ?? "epochcli"}:${Flag.EPOCHCLI_SERVER_PASSWORD}`).toString("base64")}`,
                }
              : undefined,
          ...
          const plugins = Flag.EPOCHCLI_PURE ? [] : (cfg.plugin_origins ?? [])
          if (Flag.EPOCHCLI_PURE && cfg.plugin_origins?.length) {
