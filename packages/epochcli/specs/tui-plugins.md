...
- Author package entrypoint is `@epoch-ai/plugin/tui`.
...
```json
{
  "$schema": "https://epochcli.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/epochcli-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@epoch-ai/plugin/tui`.
- `@epoch-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@epoch-ai/plugin/tui"
...
- The value is a semver range checked against the running Epoch CLI version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.epochcli` is absent, no check is performed (backward compatible).
...
- `epochcli plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `epochcli plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.epochcli` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.epochcli`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.epochcli`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.epochcli` dir or `EPOCHCLI_CONFIG_DIR`, Epoch CLI installs `@epoch-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@epoch-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.epochcli/themes` area near the plugin config source.
...
- `--pure` / `EPOCHCLI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.epochcli/plugins/tui-smoke.tsx`
- Local vim plugin: `.epochcli/plugins/tui-vim.tsx`
- Local smoke config: `.epochcli/tui.json`
- Local smoke theme: `.epochcli/plugins/smoke-theme.json`
