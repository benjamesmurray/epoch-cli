...
function linker(): "hoisted" | "isolated" {
  // jsonc-parser is only declared in packages/epochcli, so its install location
...
await Bun.build({
  ...
  define: {
    EPOCHCLI_MIGRATIONS: JSON.stringify(migrations),
    EPOCHCLI_CHANNEL: `'${Script.channel}'`,
  },
})
