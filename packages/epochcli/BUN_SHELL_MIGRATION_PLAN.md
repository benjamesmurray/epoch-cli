...
Replace runtime Bun shell template-tag usage in `packages/epochcli/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/epochcli/src` is removed except:
