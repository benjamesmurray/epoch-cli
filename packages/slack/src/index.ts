...
import { createEpoch, type ToolPart } from "@epoch-ai/sdk"
...
console.log("🚀 Starting epochcli server...")
const epochcli = await createEpoch({
  port: 0,
})
console.log("✅ Epoch server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await epochcli.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new epochcli session...")
    const { client, server } = epochcli
    ...
    console.log("✅ Created epochcli session:", createResult.data.id)
  ...
  console.log("📝 Sending to epochcli:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Epoch response:", JSON.stringify(result, null, 2))
