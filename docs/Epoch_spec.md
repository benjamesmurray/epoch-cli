...
3.4 Proactive Context Management (The Graceful Wrap-Up) To prevent catastrophic generation failure or truncated outputs caused by hitting the hard 32k context limit mid-stream, the orchestration loop employs predictive memory management managed by the Clerk model.
...
9. Persistent Functional State via MCP
...
The Spec CLI Handshake: Your Spec CLI MCP server will act as the source of truth for the workspace focus, tracking the primary and secondary concerns of the active sprint.
Idle Thinking Cycles: When the local inference engine is idle, the system can trigger an autonomous thinking cycle to update the project map TOON data or self-critique recent code generations.
