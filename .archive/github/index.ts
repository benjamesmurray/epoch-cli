...
import { createEpochClient } from "@epoch-ai/sdk"
...
const { client, server } = createEpoch()
...
function createEpoch() {
  ...
  const proc = spawn(`epochcli`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createEpochClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/epochcli|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/epochcli` or `/oc`")
  }
}
...
async function assertEpochConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to epochcli server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.epochcli.ai" : "https://epochcli.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.epochcli.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("epochcli-github-action")
    response = await fetch("https://api.epochcli.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/epochcli" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/epochcli") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to epochcli...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "epochcli-agent[bot]"`
  await $`git config --global user.email "epochcli-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `epochcli/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/epochcli-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[epochcli session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
