...
We are transitioning from a dual CLI/GUI architecture to a strictly CLI-based architecture. The core application logic and MCP integrations residing in `packages/opencode`, `packages/plugin`, `packages/sdk`, `packages/util`, and `packages/script` will be retained as the foundation. The primary entry point for users will be the Terminal User Interface (TUI) located in `packages/opencode/src/cli/cmd/tui`. 
...
- **Components to be Retained & Adjusted:**
  - `packages/opencode`: Core CLI engine, TUI, and session management.
...
Core data models managing sessions, LLM interactions, storage, and MCP communication inside `packages/opencode` will remain unchanged. Data models and state structures strictly bound to the graphical user interface (e.g., React Contexts in `packages/app`) will be removed along with their respective packages.
...
- **Retained Tests:** Unit tests, integration tests, and library-specific tests for `packages/opencode`, `packages/plugin`, and SDKs will be preserved to ensure core functionality remains intact.
