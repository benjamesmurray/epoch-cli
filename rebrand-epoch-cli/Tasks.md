...
- [ ] Rename `EpochClient` to `EpochClient` in `packages/sdk/js/src/client.ts` and `v2/client.ts`.
- [ ] Rename `createEpochClient` to `createEpochClient` in `packages/sdk/js/src/client.ts` and `v2/client.ts`.
- [ ] Rename `createEpochServer` and `createEpochTui` in `packages/sdk/js/src/server.ts` and `v2/server.ts`.
...
- [ ] Rename `EPOCHCLI_BIN_PATH` to `EPOCH_BIN_PATH` in `packages/epoch/bin/epoch`.
- [ ] Update `epochcli` to `epoch` in build scripts (e.g., `packages/epoch/package.json`).
