...
- `epochcli web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `epochcli web` view. **Do not** add for terminal, TUI, or general epochcli issues.
...
**Only** add if the issue mentions "zen" or "epochcli zen" or "epochcli black".

If the issue doesn't have "zen" or "epochcli black" in it then don't add zen label

#### core

Use for core server issues in `packages/epochcli/`, excluding `packages/epochcli/src/cli/cmd/tui/`.
...
TUI (`packages/epochcli/src/cli/cmd/tui/...`):
...
Core (`packages/epochcli/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for epochcli server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
