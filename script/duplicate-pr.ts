...
import { createEpoch } from "@epoch-ai/sdk"
...
`)
  ...
  const epochcli = await createEpoch({ port: 0 })
  ...
  try {
    ...
    const session = await epochcli.client.session.create()
    const result = await epochcli.client.session
      .prompt({
