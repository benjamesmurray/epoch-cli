...
const model = "epochcli/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/epochcli")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/epochcli")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with epochcli...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/epochcli`.",
  ...
  try {
    await $`epochcli run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  epochcli failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with epochcli")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with epochcli...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/epochcli`.",
    "Run `./script/build.ts --single` in `packages/epochcli`.",
