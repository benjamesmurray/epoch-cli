...
const FORK_REPO = "benjamesmurray/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "epochcli"
...
async function main() {
  ...
  // Delete any existing branches for epochcli updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating Epoch CLI extension to v${cleanVersion}"`
