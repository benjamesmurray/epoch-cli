...
3. **Guidance Loop**: Project tools (`mcp-spec-cli`, etc.) detect or default to recommending `mcpx` syntax in their "Next Step" outputs.
...
### 2. Spec CLI (`mcp-spec-cli`)
- **`src/features/shared/SpecManager.ts`**: Update the `nextSteps` generator to prefix commands with `mcpx mcp-spec-cli`.
...
- **`Dockerfile.eval`**: Pre-install `mcpx-go`, `mcp-spec-cli`, and `ground-truth-cli` from local sources to ensure binary availability.
...
- **Automated E2E**: 
    ...
    - Verification: The `eventbus-v2` test must pass, and the `run.log` must show the agent successfully using `mcpx mcp-spec-cli ...` commands.
